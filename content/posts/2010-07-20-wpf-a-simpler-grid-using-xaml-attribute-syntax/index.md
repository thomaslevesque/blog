...
In XAML, all attributes are written in the form of strings. To convert these strings to values of the required type, WPF makes use of converters, which are classes derived from `TypeConverter`, associated with each type which supports conversion to and from other types. For instance, the converter for the `GridLength` structure is the `GridLengthConverter` class, which can convert numbers and strings to `GridLength` objects, and back. The conversion mechanism is described in more detail in [this MSDN article](http://msdn.microsoft.com/en-us/library/aa970913.aspx).
