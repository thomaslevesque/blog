...
***Note:** The code in this article is out of date; the current code is [hosted on GitHub](https://github.com/XamlAnimatedGif/WpfAnimatedGif/).*
...
The next problem is to extract the frames from the image: fortunately WPF supports this natively, and the [`BitmapDecoder`](http://msdn.microsoft.com/en-us/library/system.windows.media.imaging.bitmapdecoder.aspx) class provides a `Frames` property to do exactly that. So, no big difficulty so far...
